 ...
 * 
 */
public abstract class AbstractFloatSpectralDeconvolver3D implements FloatSpectralDeconvolver {
    ...
    /**
     * Creates new instance of AbstractFloatSpectralDeconvolver3D
     * 
     * @param name
     ...
     */
    public AbstractFloatSpectralDeconvolver3D(String name, ImagePlus imB, ImagePlus imPSF, ResizingType resizing, OutputType output, PaddingType padding, boolean showPadded, float regParam, float threshold) {
        ...
        FloatCommon3D.assignPixelsToMatrix(isB, (FloatMatrix3D) B);
        if (isPadded) {
            switch (padding) {
            case PERIODIC:
                B = FloatCommon3D.padPeriodic((FloatMatrix3D) B, bSlicesPad, bRowsPad, bColumnsPad);
                break;
            case REFLEXIVE:
                B = FloatCommon3D.padReflexive((FloatMatrix3D) B, bSlicesPad, bRowsPad, bColumnsPad);
                break;
            default:
        ...
        FloatCommon3D.assignPixelsToMatrix(isPSF, (FloatMatrix3D) PSF);
        ...
        if (kSlices != bSlicesPad || kRows != bRowsPad || kCols != bColumnsPad) {
            PSF = FloatCommon3D.padZero((FloatMatrix3D) PSF, bSlicesPad, bRowsPad, bColumnsPad);
        }
        ...
        if (showPadded && isPadded) {
            ...
            FloatCommon3D.assignPixelsToStack(stackTemp, (FloatMatrix3D) B, cmY);
