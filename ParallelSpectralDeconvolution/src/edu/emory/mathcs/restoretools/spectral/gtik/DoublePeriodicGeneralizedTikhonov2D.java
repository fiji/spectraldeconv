 ...
 */
package edu.emory.mathcs.restoretools.spectral.gtik;
...
import edu.emory.mathcs.restoretools.spectral.AbstractDoubleSpectralDeconvolver2D;
...
public class DoublePeriodicGeneralizedTikhonov2D extends AbstractDoubleSpectralDeconvolver2D {
     ...
     */
    public DoublePeriodicGeneralizedTikhonov2D(ImagePlus imB, ImagePlus imPSF, DoubleMatrix2D stencil, ResizingType resizing, OutputType output, boolean showPadded, double regParam, double threshold) {
    ...
    public ImagePlus deconvolve() {
        ...
        if (threshold == -1.0) {
            if (isPadded) {
                DoubleCommon2D.assignPixelsToProcessorPadded(ip, (DComplexMatrix2D) Sa, bRows, bColumns, bRowsOff, bColumnsOff, cmY);
            } else {
                DoubleCommon2D.assignPixelsToProcessor(ip, (DComplexMatrix2D) Sa, cmY);
            }
        } else {
            if (isPadded) {
                DoubleCommon2D.assignPixelsToProcessorPadded(ip, (DComplexMatrix2D) Sa, bRows, bColumns, bRowsOff, bColumnsOff, cmY, threshold);
            } else {
                DoubleCommon2D.assignPixelsToProcessor(ip, (DComplexMatrix2D) Sa, cmY, threshold);
            }
        }
